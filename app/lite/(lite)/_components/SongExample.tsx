...
export default function SongExample() {
    ...
    const gotoExampleSong = (genius_id: string, music_brainz_id: string, high_level: HighLevelData, low_level: LowLevelData, genius: GeniusFormattedData) => {
        ...
        router.push(`/lite/chat/${genius_id}/${music_brainz_id}`);
    }
        ...
        <div className='border-secondary border bg-secondary-container rounded-b-lg'>
            {exampleSongs.map((song, index) => (
                <div key={index}>
                    ...
                    <Artist onClick={() => gotoExampleSong(song.geniusID, song.musicBrainzID, song.highLevel, song.lowLevel, song.genius)} style={Style.lite} artist={song.artist} title={song.title} duration={song.duration} year={song.year} imageUrl={song.imageUrl} />
                </div>
            ))}
        </div>
    </div>
