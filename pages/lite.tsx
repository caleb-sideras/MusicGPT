...
import { Lite } from '@/components/Features/Features';
    ...
    return (
        <>
            ...
            <div className="flex flex-col gap-4 sm:px-10 pb-4 sm:pb-10 max-w-[800px] mx-auto sm:mt-4">
                ...
                <div>
                    <div className='flex flex-col gap-2 p-4 text-on-surface w-full h-fill bg-secondary rounded-t-lg'>
                    ...
                    <div className='border-secondary border bg-secondary-container rounded-b-lg'>
                        {exampleSongs.map((song, index) => (
                            <>
                                <div className="border-t border-secondary w-full" />
                                <Artist key={index} onClick={() => gotoExampleSong(song.geniusID, song.musicBrainzID, song.highLevel, song.lowLevel, song.genius)} style={Style.lite} artist={song.artist} title={song.title} duration={song.duration} year={song.year} imageUrl={song.imageUrl} />
                            </>
                        ))}
                    </div>
                </div>
                <Link href='/lite/search'>
                    <div className='w-full p-4 text-center text-secondary hover:text-on-secondary hover:text-surface rounded-full bg-surface hover:bg-secondary border-secondary hover:border-surface border-2 transition cursor-pointer'>
                        Search
                    </div>
                </Link>
            </div>
